...
const VendorTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => (
        <img
          src={`${process.env.REACT_APP_BACKEND_URL}/${row.vendor_img}`}
    ...
    {
      name: "EMAIL",
      selector: (row) => row.vender_email,
      sortable: true,
    },
    {
      name: "START DATE",
      selector: (row) => row.date_of_start,
      sortable: true,
    },
    ...
    {
      name: "WEB ADDRESS",
      selector: (row) => row.websiteAddress,
      sortable: true,
    },
    {
      name: "ADDRESS",
   ...
   
    {
      name: "ACTION",
      cell: (row) => (
        <NavLink to={{ pathname: `/vendor/vendorUpdateForm/${row.vendor_id}` }}>
    ...
    },
    {
      cell: (row) => (
        <button
        className="de_btn"
        onClick={() => deleteuser(row.vendor_id)}
  ...
  useEffect(() => {
    const result = users.filter((user) => {
      return (
        user.Gstin?.toLowerCase().match(search.toLowerCase()) ||
        user.org_name?.toLowerCase().match(search.toLowerCase()) ||
        user.status?.toLowerCase().match(search.toLowerCase()) ||
        user.mobile_number?.toLowerCase().match(search.toLowerCase()) ||
        user.vender_email?.toLowerCase().match(search.toLowerCase()) ||
        user.date_of_start?.toLowerCase().match(search.toLowerCase()) ||
        user.registered?.toLowerCase().match(search.toLowerCase()) ||
        user.websiteAddress?.toLowerCase().match(search.toLowerCase()) ||
        user.address?.toLowerCase().match(search.toLowerCase()) ||
        user.city?.toLowerCase().match(search.toLowerCase()) ||
       user.state?.toLowerCase().match(search.toLowerCase())
         );
    });
  ...
  const getUsers = async () => {
    const response = await axios.get(
       `${process.env.REACT_APP_BACKEND_URL}/Vendor/allVendor`,
      {
        headers: {
    ...
    console.log("getAll_TrekData=====>",response.data.allVendor)
    setUsers(response.data.allVendor);
    setFilteruser(response.data.allVendor);
    setLoading(false);
  };
  ...
  const deleteConfirmed = async (id) => {
   var deleteVendor =  await axios.delete(
         `${process.env.REACT_APP_BACKEND_URL}/Vendor/deleteVendor/${user_id}`,
         {
           headers: {
      ...
      <div className="col-lg-12 grid-margin stretch-card card_pad">
        <div className="card">
          <div className="table1_a mt1">
            <div className="table2_b">Vendor Data</div>
          </div>
