...
const TrailingTable = () => {
  ...
  const [trailing_id, setSelectedUserId] = useState(null);
  ...
  const columns = [
    {
      ...
      cell: (row) => {
          return <div>{row.trailing_id}</div>;
        },
    },

    {
      ...
      selector: (row) => (
        <img
          src={`${process.env.REACT_APP_BACKEND_URL}/${row.trailing_img}`}
    ...
    {
      name: "NAME",
      selector: (row) => row.trailingName,
      sortable: true,
    },
    {
      name: "STATUS",
      selector: (row) => row.trailingStatus,
      sortable: true,
    },
    {
      name: "ACTION",
      cell: (row) => (
        <NavLink
          to={{ pathname: `/trailing/trailingUpdateForm/${row.trailing_id}` }}
        >
    ...
    },
    {
      cell: (row) => (
        <button
          className="de_btn"
          onClick={() => deleteuser(row.trailing_id)}
  ...
  useEffect(() => {
    const result = users.filter((user) => {
      return (
        user.trailingStatus.toLowerCase().match(search.toLowerCase()) ||
        user.trailingName.toLowerCase().match(search.toLowerCase())
      );
    });
  ...
  const getUsers = async () => {
    ...
    setUsers(response.data.alltrailing);
    setFilteruser(response.data.alltrailing);
    setLoading(false);
  };
  ...
  const deleteConfirmed = async () => {
      await axios.delete(
        `${process.env.REACT_APP_BACKEND_URL}/Trailing/deleteTrailing/${trailing_id}`,
        {
          headers: {
  ...
  const Add_record = () => {
   navigate("/trailing/trailingAddForm");
  };
    ...
    <div className="col-lg-12 grid-margin stretch-card card_pad">
      <div className="card ">
        <div className="table1_a mt1">
          <div class="table2_b">Trailing Data </div>
          </div>
