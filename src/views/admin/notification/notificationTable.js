...
const NotificationTable = () => {
  ...
  const [notifiaction_id, setSelectedUserId] = useState(null);
  ...
  const columns = [
    ...
    //   <button
    //       className="de_btn"
    //       onClick={() => deleteuser(row.notifiaction_id)}
  ...
  useEffect(() => {
    const result = users.filter((user) => {
      return user.title.toLowerCase().match(search.toLowerCase()) ||
       user.message.toLowerCase().match(search.toLowerCase()) ||
       user.user_id.toLowerCase().match(search.toLowerCase());
    });
  ...
  const getUsers = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/Notification/all_notification`,
      {
        headers: {
  ...
  const deleteConfirmed = async (id) => {
      await axios.delete(
        `${process.env.REACT_APP_BACKEND_URL}/Notification/delete_notification/${notifiaction_id}`,
        {
          headers: {
  ...
  const Add_record = () => {
    navigate("/notification/notificationAddForm");
  };
    ...
    <div className="col-lg-12 grid-margin stretch-card card_pad">
      <div className="card">
        <div className="table1_a mt1">
          <div class="table2_b">Notification Data </div>
        </div>
