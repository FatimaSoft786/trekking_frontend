...
const ItineraryTable = () => {
  ...
  const columns = [
    ...
    //     selector: (row) => (
    //       <img
    //         src={`${process.env.REACT_APP_BACKEND_URL}/${row.icon_img}`}
    ...
    {
      name: "ICON NAME",
      selector: (row) => row.icon_id,
      sortable: true,
    },
    {
      name: "DAY NAME",
      selector: (row) => row.day_name + " Day",
      sortable: true,
    },
    {
      name: "DESCRIPTION",
    ...
    },
    {
      ...
      cell: (row) => (
        <NavLink
          to={{ pathname: `/itinerary/itineraryUpdateForm/${row.itinerary_id}` }}
        >
    ...
    },
    {
      cell: (row) => (
        <button
          className="de_btn"
          onClick={() => deleteuser(row.itinerary_id)}
  ...
  useEffect(() => {
    const result = users.filter((user) => {
      return (
        user.icon_id.toLowerCase().match(search.toLowerCase()) ||
        user.description.toLowerCase().match(search.toLowerCase()) ||
        user.day_name.toLowerCase().match(search.toLowerCase())
      );
    });
  ...
  const getUsers = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/Itinerary/all_Itinerary`,
      {
        headers: {
    ...
    setUsers(response.data.allItinerary);
    setFilteruser(response.data.allItinerary);
    setLoading(false);
  };
  ...
  const deleteConfirmed = async () => {
      await axios.delete(
        `${process.env.REACT_APP_BACKEND_URL}/Itinerary/DeleteItinerary/${category_id}`,
        {
          headers: {
  ...
  const Add_record = () => {
   navigate("/itinerary/itineraryAddForm");
  };
    ...
    <div className="col-lg-12 grid-margin stretch-card card_pad">
      <div className="card ">
        <div className="table1_a mt1">
          <div class="table2_b">Itinerary Data </div>
          </div>
