...
const CategoryUpdateForm = () => {
  ...
  const updateuser = async (e) => {
    ...
    const resp = await axios.put(
      `${process.env.REACT_APP_BACKEND_URL}/Category/updateCategory/${id}`,
      fromData,
      {
        headers: {
  ...
  useEffect(() => {
    const getProductById = async () => {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/Category/oneCategory/${id}`,
        {
          headers: {
            Authorization: `Bearer ${data.accessToken}`,
          },
      ...
      setCategoryName(response.data.getById.categoryName);
      setCategoryStatus(response.data.getById.categoryStatus);
      setCategory_img(response.data.getById.category_img);
      setProfile_img1(response.data.getById.category_img);
      setProfile_img2(response.data.getById.category_img);
       };
    getProductById();
  }, [id]);
  useEffect(() => {
          ...
          <div className="card-body card_b1">
            ...
            <div class="row">
                ...
                <div className="image-upload-container">
                    <label htmlFor="category_img" className="image-upload-icon">
                   {category_img_url ? (
                      <div className="image-preview">
                        <img src={category_img_url} alt="Preview" className="imgBox2" />
                      </div>
                    ) : (
                      <>
