...
const CategoryTable = () => {
  ...
  const columns = [
    {
      ...
      cell: (row) => {
          return <div>{row.category_id}</div>;
        },
    },

    {
      ...
      selector: (row) => (
        <img
          src={`${process.env.REACT_APP_BACKEND_URL}/${row.category_img}`}
    ...
    },
    {
      ...
      cell: (row) => (
        <NavLink
          to={{ pathname: `/category/categoryUpdateForm/${row.category_id}` }}
        >
    ...
    },
    {
      cell: (row) => (
        <button
          className="de_btn"
          onClick={() => deleteuser(row.category_id)}
  ...
  useEffect(() => {
    const result = users.filter((user) => {
      return (
        user.categoryName.toLowerCase().match(search.toLowerCase()) ||
        user.categoryStatus.toLowerCase().match(search.toLowerCase())
      );
    });
  ...
  const getUsers = async () => {
    ...
    setUsers(response.data.allCategory);
    setFilteruser(response.data.allCategory);
    setLoading(false);
  };
    ...
    <div className="col-lg-12 grid-margin stretch-card card_pad">
      <div className="card ">
        <div className="table1_a mt1">
          <div class="table2_b">Category Data </div>
          </div>
