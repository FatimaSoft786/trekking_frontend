...
const LandscapeTable = () => {
  ...
  const columns = [
    {
     ...
     // cell: (row, index) => index + 1,
      cell: (row) => {
          return <div>{row.landscape_id}</div>;
        },
    },

    {
      ...
      selector: (row) => (
        <img
          src={`${process.env.REACT_APP_BACKEND_URL}/${row.landscape_img}`}
    ...
    {
      name: "NAME",
      selector: (row) => row.landscapeName,
      sortable: true,
    },
    {
      name: "STATUS",
      selector: (row) => row.landscapeStatus,
      sortable: true,
    },
    {
      name: "ACTION",
      cell: (row) => (
        <NavLink
          to={{ pathname: `/landscape/landscapeUpdateForm/${row.landscape_id}` }}
        >
  ...
  useEffect(() => {
    const result = users.filter((user) => {
      return (
        user.landscapeName.toLowerCase().match(search.toLowerCase()) ||
        user.landscapeStatus.toLowerCase().match(search.toLowerCase())
      );
    });
  ...
  const getUsers = async () => {
    ...
    setUsers(response.data.allLandscape);
    setFilteruser(response.data.allLandscape);
    setLoading(false);
  };
    ...
    <div className="col-lg-12 grid-margin stretch-card card_pad">
      <div className="card ">
        <div className="table1_a mt1">
          <div class="table2_b">Landscape Data </div>
          </div>
