...
const UserTable = () => {
  ...
  const columns = [
...

    {
      ...
      selector: (row) => (
        <img
          src={`${process.env.REACT_APP_BACKEND_URL}/${row.profile_img}`}
   ...
   
    {
      name: "ACTION",
      cell: (row) => (
        <NavLink to={{ pathname: `/user/userUpdateForm/${row.user_id}` }}>
  ...
  useEffect(() => {
    const result = users.filter((user) => {
      return (
        user.first_name?.toLowerCase().match(search.toLowerCase()) ||
        user.last_name?.toLowerCase().match(search.toLowerCase()) ||
        ...
        user.user_DOB?.toLowerCase().match(search.toLowerCase()) ||
  ...
  const getUsers = async () => {
    const response = await axios.get(
       `${process.env.REACT_APP_BACKEND_URL}/Users/allUser`,
      {
        headers: {
  ...
  const deleteConfirmed = async (id) => {
    await axios.delete(
         `${process.env.REACT_APP_BACKEND_URL}/Users/userDelete/${user_id}`,
         {
           headers: {
      ...
      <div className="col-lg-12 grid-margin stretch-card card_pad">
        <div className="card">
          <div className="table1_a mt1">
            <div className="table2_b">User Data</div>
          </div>
