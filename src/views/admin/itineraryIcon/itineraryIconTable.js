...
const ItineraryIconTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <NavLink
          to={{ pathname: `/itineraryIcon/itineraryIconUpdateForm/${row.icon_id}` }}
        >
    ...
    },
    {
      cell: (row) => (
        <button
          className="de_btn"
          onClick={() => deleteuser(row.icon_id)}
  ...
  useEffect(() => {
    const result = users.filter((user) => {
      return (
        user.iconName.toLowerCase().match(search.toLowerCase()) ||
        user.iconStatus.toLowerCase().match(search.toLowerCase())
      );
    });
  ...
  const getUsers = async () => {
    ...
    setUsers(response.data.allIcon);
    setFilteruser(response.data.allIcon);
    setLoading(false);
  };
  ...
  const deleteConfirmed = async () => {
      await axios.delete(
        `${process.env.REACT_APP_BACKEND_URL}/Icon/deleteIcon/${category_id}`,
        {
          headers: {
  ...
  const Add_record = () => {
   navigate("/itineraryIcon/itineraryIconAddForm");
  };
    ...
    <div className="col-lg-12 grid-margin stretch-card card_pad">
      <div className="card ">
        <div className="table1_a mt1">
          <div class="table2_b">Itinerary Icon Data </div>
          </div>
...
export default ItineraryIconTable;
